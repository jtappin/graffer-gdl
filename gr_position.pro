...
case but of
    ...
    'PXMIN': begin
        ...
        if (event.cr) then grf_focus_enter, uv.pids(1)
    end                                     
                                            
    'PYMIN': begin                          
        uv.pos(1) = event.value             
        if (event.cr) then grf_focus_enter, uv.pids(2)
    end                                     
                                            
    'PXMAX': begin                          
        uv.pos(2) = event.value             
        if (event.cr) then grf_focus_enter, uv.pids(3)
    end
    
    'PYMAX': begin
        uv.pos(3) = event.value
        if (event.cr) then grf_focus_enter, uv.pids(0)
    end
    
    'ASPECT': begin                          
        uv.asp(0) = event.value             
        if (event.cr) then grf_focus_enter, uv.rids(1)
    end
    
    'MARGIN': begin                          
        uv.asp(1) = event.value             
        if (event.cr) then grf_focus_enter, uv.rids(0)
    end
    'ISO': begin
        uv.iso = event.select
        iflag = 1b
...
pids(0) = graff_enter(jb, /all_events, /float, value = pdefs.position(0), $
                      format = "(f6.4)", uvalue = 'PXMIN', label = $
                      'Lower left, X:', xsize = 6, /capture)
pids(1) = graff_enter(jb, /all_events, /float, value = pdefs.position(1), $
                      format = "(f6.4)", uvalue = 'PYMIN', label = ' Y:', $
                      xsize = 6, /capture)

jb = widget_base(pbase, /row, xpad = 0, ypad = 0, space = 0)
pids(2) = graff_enter(jb, /all_events, /float, value = pdefs.position(2), $
                      format = "(f6.4)", uvalue = 'PXMAX', label = $
                      'Upper right, X:', xsize = 6, /capture)
pids(3) = graff_enter(jb, /all_events, /float, value = pdefs.position(3), $
                      format = "(f6.4)", uvalue = 'PYMAX', label = ' Y:', $
                      xsize = 6, /capture)
...
rids(0) = graff_enter(jb, /all_events, /float, value = pdefs.aspect(0), $
                      format = "(f8.4)", uvalue = 'ASPECT', label = $
                      'Aspect ratio:', xsize = 8, /capture)
rids(1) = graff_enter(jb, /all_events, /float, value = pdefs.aspect(1), $
