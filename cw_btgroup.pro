; $Id: cw_btgroup.pro,v 1.13 1995/04/11 18:12:50 ali Exp $
...
function CW_BTGROUP_GETV, id, value
...
function CW_BTGROUP_EVENT, ev
  ...
  if ret then begin		;Return a struct?
      e_type = tag_names(ev, /struct)
      if (e_type eq 'WIDGET_TRACKING') then $
        ret = {CW_BG_TRACK, ID:state.base, TOP:ev.top, HANDLER:0L, $
               Enter:ev.enter, Value:state.ret_arr(uvalue)} $
      else ret = {CW_BG, ID:state.base, TOP:ev.top, HANDLER:0L, $
                  Select:ev.select, Value:state.ret_arr(uvalue)}
      efun = state.efun
      WIDGET_CONTROL, stash, SET_UVALUE=state, /NO_COPY
...
function CW_BTGROUP, parent, names, $
                    BUTTON_UVALUE=button_uvalue, COLUMN=column, $
                    EVENT_FUNCT=efun, EXCLUSIVE=excl, FONT=font, $
                    FRAME=frame, IDS=ids, LABEL_TOP=label_top, $ 
                    LABEL_LEFT=label_left, MAP=map, $
                    NONEXCLUSIVE=nonexcl, NO_RELEASE=no_release, $
                    RETURN_ID=return_id, RETURN_INDEX=return_index, $
                    RETURN_NAME=return_name, ROW=row, SCROLL=scroll, $
                    SET_VALUE=sval, SPACE=space, UVALUE=uvalue, $ 
  ...
  WIDGET_CONTROL, top_base, MAP=map, $
	FUNC_GET_VALUE='CW_BTGROUP_GETV', PRO_SET_VALUE='CW_BTGROUP_SETV', $
	SET_UVALUE=uvalue
  ...
  if (n_elements(sval) ne 0) then CW_BTGROUP_SETV, top_base, sval
