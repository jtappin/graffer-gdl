...
(\textsc{\autoref{hardcopy menu}}).
 ...
 \item [Normal/Encapsulated:]Normal is just a regular printable
   ...
   into other files e.g. xfig\texttt{,} \LaTeX etc. The filename ends
   \texttt{.ps} or \texttt{.eps} according to the selected option.
...
\item[Default mouse editing] Controls whether mouse editing is allow on
  new datasets.
\item[Colour selector] Whether the colour selector should be text-based
  or show samples. N.B. This is only useful with the ``Save'' option as
  it cannot be changed once GRAFFER has been started.
