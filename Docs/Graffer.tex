...
\url{https://github.com/jtappin/graffer-idl}.
So if you want to download it to \texttt{/contrib/idl/graffer-idl},
then:
\begin{verbatim}
$ cd /contrib/idl/
$ git clone https://github.com/jtappin/graffer-idl.git
\end{verbatim}
...
\item [Contoured~display:]The contouring menu
  ...
  \item [Levels:]The levels can be either explicit, in which case the
    ...
    other options, so it is done by GRAFFER\footnote{For real
      enthusiasts, the levels are determined as \texttt{levels = rg
        {*} (findgen(nl)+.5)/nl + mn} where \texttt{rg} is the range
      of the data (ignoring infinities \& NaNs), \texttt{mn} is the
      smallest value in the data and \texttt{nl} is the number of
      levels.}.
