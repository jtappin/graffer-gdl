...
;	Reasonably working version,  more like IDL: 15/9/21; SJT
;-
...
; Full path
     ...
     if dname ne uvs.path then begin
        ...
        if ptr_valid(uvs.list) then ptr_free, uvs.list

        if ndir+nfile eq 0 then begin
           uvs.list = ptr_new('')
           uvs.isdir = ptr_new(0b)
           widget_control, uvs.flistid, set_value = '', sensitive = 0
        endif else begin
           llist = strarr(ndir+nfile)
           isdir = bytarr(ndir+nfile)
           ...
           tlist = llist
           uvs.list = ptr_new(llist)
           uvs.isdir = ptr_new(isdir)
           if ndir ne 0 then tlist[0:ndir-1] += path_sep()
           widget_control, uvs.flistid, set_value = tlist, /sensitive
        endelse
     endif
...
function cw_fs_event, event
  ...
  case but of
     ...
     'FILES': begin
        cfile = (*uvs.list)[event.index]
        dir = (*uvs.isdir)[event.index]
        if ~dir then begin
           widget_control, uvs.fileid, set_value = cfile
           if event.clicks eq 2 then done = 1
           uvs.sflag = 1b
        endif else begin
           uvs.path = uvs.path+path_sep()+cfile
           widget_control, uvs.fileid, set_value = ''
     ...
     'FILE': begin
        ...
        locs = where(*uvs.list eq cfile, nm) 
        if nm eq 0 then widget_control, uvs.flistid, $
                                        set_list_select = -1 $
        else begin
           if (*uvs.isdir)[locs[0]] then begin
              lsl = widget_info(uvs.flistid, /list_select)
              if lsl[0] ne -1 then $
                 widget_control, event.id, set_value = $
                                 (*uvs.list)[lsl[0]] $
              else widget_control, event.id, set_value = ''
           endif else $
              widget_control, uvs.flistid, $
...
function cw_filesel, parent, $
  ...
  if keyword_set(fix_filter) then $
     filtid = widget_text(cb, $
                          value = filter[0]) $
  else $
  ...
  if ndir+nfile eq 0 then begin
     isdir = 0b
     llist = ''
     widget_control, flistid, set_value = llist, sensitive = 0
  endif else begin
     isdir = bytarr(ndir+nfile)
     llist = strarr(ndir+nfile)
     if ndir ne 0 then begin
     ...
     tlist = llist
     if ndir ne 0 then tlist[0:ndir-1] += path_sep()
     widget_control, flistid, set_value = tlist, sensitive = 1
  endelse
